...
const AdminContactPage: React.FC = () => {
  ...
  const visibleContacts = pendingDelete
    ? contacts.filter((c) => c._id !== pendingDelete)
    : contacts;

  const filteredContacts =
    filter === "all"
      ? visibleContacts
      : visibleContacts.filter((c) => c.status === filter);
  ...
  const handleReply = async () => {
    ...
    try {
      const response = await axios.post(
        `${API_BASE}/contacts/${selected._id}/reply`,
        { reply }
      );

      let toastMessage = "Đã gửi phản hồi cho khách hàng!";

      // Kiểm tra trạng thái email
      if (response.data.emailStatus === "sent") {
        toastMessage += " ✅ Email đã được gửi thành công.";
      } else if (response.data.emailStatus === "failed") {
        toastMessage += " ⚠️ Email gửi thất bại, nhưng phản hồi đã được lưu.";
      } else if (response.data.emailStatus === "not_sent") {
        toastMessage += " ℹ️ Không có email để gửi.";
      }
  ...
  const handleClose = async () => {
    if (!selected) return;
    try {
      await axios.patch(`${API_BASE}/contacts/${selected._id}/status`, {
        status: "closed",
      });
  ...
  return (
    <div className="admin-contact-page">
      ...
      <div
        ...
        style={{ justifyContent: "flex-end", display: "flex", marginTop: 16 }}
      >
        <button
        ...
        >
          <div className="contact-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              ...
              {selected.status === "pending" ? (
                ...
                <div className="reply-content">
                  {selected.reply || <i>Chưa có phản hồi</i>}
                </div>
              )}
            </div>
            <div className="modal-footer">
              ...
              {(selected.status === "pending" ||
                selected.status === "replied") && (
                <button className="btn-close-contact" onClick={handleClose}>
                  Đóng liên hệ
                </button>
              )}
              {selected.status === "closed" && (
                <button className="btn-open-contact" onClick={handleOpen}>
                  Mở lại liên hệ
                </button>
              )}
              <button className="btn-delete-contact" onClick={handleDelete}>
                Xóa liên hệ
              </button>
            </div>
          </div>
      ...
      {toast && (
        <div className="contact-toast-popup">
          {pendingDelete
            ...
            : toast}
          {pendingDelete && (
            <button className="btn-undo-delete" onClick={handleUndoDelete}>
              Hoàn tác
            </button>
          )}
        </div>
